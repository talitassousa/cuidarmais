...
class ConsultasControllerTest {
    ...
    @Test
    @DisplayName("Deveria devolver codigo 400 quando as informaçoes estão inválidas")
    @WithMockUser
    void erro400agendar() throws Exception {
    ...
    @Test
    @DisplayName("Deveria devolver codigo 200 quando as informaçoes estão válidas")
    @WithMockUser
    void sucesso200agendar() throws Exception {
        var data = LocalDateTime.now().plusHours(1);
        var especialidade = Especialidade.CARDIOLOGIA;

        var dadosDetalhamento = new DadosDetalhamentoConsulta(null, 2l, 5l, data);
        when(agendarConsultasService.agendar(any())).thenReturn(dadosDetalhamento);
